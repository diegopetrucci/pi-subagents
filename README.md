...
Press the fork-owned `Ctrl+Shift+D` shortcut to toggle full subagent detail. Its extension-session state owns both running and completed foreground subagent rows and is independent of Pi's global `Ctrl+O` tool-output expansion; completed background notifications continue to use Pi's configured expand key.
