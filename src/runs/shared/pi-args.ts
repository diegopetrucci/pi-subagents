...
export function buildPiArgs(input: BuildPiArgsInput): BuildPiArgsResult {
	...
	// Always set the "__none__" sentinel for @diegopetrucci/pi-mcp-adapter (bundled in TLH).
	// The adapter's init.ts checks `envDirect !== "__none__"` before bootstrapping direct MCP tools;
	// an unset var means "bootstrap everything configured", which would silently widen every child's tool surface.
