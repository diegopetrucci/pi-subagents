...
const SubagentParamsSchema = Type.Object({
	...
	timeoutMs: Type.Optional(Type.Integer({ minimum: 1, description: "Foreground-only hard cancellation deadline in ms. When reached, the child run is interrupted/terminated rather than merely waited on. Use status/resume for async/background runs. Alias of maxRuntimeMs." })),
	maxRuntimeMs: Type.Optional(Type.Integer({ minimum: 1, description: "Alias of timeoutMs: a foreground-only hard cancellation deadline in ms that interrupts/terminates the child run, not a normal wait budget. Use status/resume for async/background runs." })),
