...
### Removed
- Removed the `wait` tool (`src/runs/background/wait.ts`, 390 LOC) and all wiring: `WaitParamsSchema`/`WaitParams` from `src/extension/schemas.ts`, `WaitToolConfigObject`/`WaitToolConfig`/`waitTool` config field from `src/shared/types.ts`, and the `waitToolConfig` const plus `registerTool(waitTool)` call from `src/extension/index.ts` (ps-7lre, #125 Group A slice 3). This is a **user-visible removal** for any non-TLH consumer — the `wait` tool was registered unconditionally and was callable by any parent agent that had access to it. TLH could not reach it: TLH's primary-agent tool allowlist excludes it, no TLH agent declares it, and TLH never sets `config.waitTool` or `PI_SUBAGENT_WAIT_TOOL_ENABLED`. Removing it also makes the subagent tool description more accurate for TLH agents, which were being instructed to call a tool they could not reach.
- Removed unsupported Windows CI from the Tests workflow. (psn-fghs)
- Removed the `mcpDirectTools` allowlist resolver (`src/runs/shared/mcp-direct-tool-allowlist.ts`, 365 LOC) and the `mcpDirectTools?: string[]` frontmatter field threading from all 11 call-site files (ps-o8m8, #125 Group A slice 2 part 2): no TLH agent declares the field, and TLH is the only consumer. **Kept**: `env.MCP_DIRECT_TOOLS = "__none__"` is still set unconditionally in `src/runs/shared/pi-args.ts`; the sentinel tells `@diegopetrucci/pi-mcp-adapter` (bundled in TLH) not to bootstrap direct MCP tools in child subagents — an unset var means "bootstrap everything configured" and would silently widen every child's tool surface. This sentinel must not be removed by a future cleanup pass.
- Removed the `agent-memory` module (`src/agents/agent-memory.ts`) and its test (`test/unit/agent-memory.test.ts`) (ps-rjqf, #125 Group A slice 2 part 1): no TLH agent declares the `memory` frontmatter field, and TLH is this fork's only consumer. The `AgentMemoryScope` type, `AgentMemoryConfig` interface, and `memory` field are removed from `AgentConfig` in `src/agents/agents.ts`; the three `buildAgentMemoryInjection` call sites are removed from `src/runs/foreground/execution.ts` and `src/runs/background/async-execution.ts`. The `memory` frontmatter row and Per-agent persistent memory section are removed from `README.md`.
